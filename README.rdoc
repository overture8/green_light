...
1. Run this generator to copy the required assets to your public folder:-

  rails g green_light_assets

2. Insert the following javascript include tag in your layout html header:-
  
  <%= javascript_include_tag 'jquery-1.4.4.min', 'jquery.validate.min', 'jquery.validate_regex', 'green_light' %>  

3. Add the <tt>validate_with_ajax</tt> class to the forms that you wish to have client side validation:-

  <%= form_for(@model, :html => { :class => 'validate_with_ajax' }) do |f| %>

4. Add some validations to your models!
